...
def parse_tagtxt_file_to_csv(infile, outfile):
    ...
    outfile.write(parser.session.get_statements_csv())
    outfile.close()
...
if __name__ == '__main__':
    ...
    if not output:
        if not input:
            ...
            if os.path.isfile(input):
                ...
                output = input.replace('.tag.txt', '.csv')
            else:
    ...
    if os.path.isdir(input):
        ...
        for f in glob.glob(os.path.join(input, '*.txt')):
            if output:
                inputs[f] = os.path.join(output, os.path.basename(f).replace('.tag.txt', '.csv'))
            else:
                # sem output -> grava o txt no mesmo dir
                inputs[f] = os.path.join(input, os.path.basename(f).replace('.tag.txt', '.csv'))
        for i in inputs:
